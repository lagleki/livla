...
<xsl:template name="delatex">
  ...
  <xsl:choose>
    ...
    <xsl:when test="contains($string, '$x_{1}$')">
      <xsl:value-of select="substring-before($string, '$x_{1}$')" />
      <em>x1</em>
      <xsl:call-template name="delatex">
        <xsl:with-param name="string" select="substring-after($string, '$x_{1}$')" />
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="contains($string, '$x_{2}$')">
      <xsl:value-of select="substring-before($string, '$x_{2}$')" />
      <em>x2</em>
      <xsl:call-template name="delatex">
        <xsl:with-param name="string" select="substring-after($string, '$x_{2}$')" />
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="contains($string, '$x_{3}$')">
      <xsl:value-of select="substring-before($string, '$x_{3}$')" />
      <em>x3</em>
      <xsl:call-template name="delatex">
        <xsl:with-param name="string" select="substring-after($string, '$x_{3}$')" />
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="contains($string, '$x_{4}$')">
      <xsl:value-of select="substring-before($string, '$x_{4}$')" />
      <em>x4</em>
      <xsl:call-template name="delatex">
        <xsl:with-param name="string" select="substring-after($string, '$x_{4}$')" />
      </xsl:call-template>
    </xsl:when>
    <xsl:when test="contains($string, '$x_{5}$')">
      <xsl:value-of select="substring-before($string, '$x_{5}$')" />
      <em>x5</em>
      <xsl:call-template name="delatex">
        <xsl:with-param name="string" select="substring-after($string, '$x_{5}$')" />
      </xsl:call-template>
    </xsl:when>
