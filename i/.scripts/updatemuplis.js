...
function selectinto() {
  ...
  db.serialize(() => {
    ...
    db.each("SELECT tat.sentence as one,tati.sentence as six,tat.lang as lan FROM tatoeba as tat left join links on links.fir=tat.idsentence left join tatoeba as tati on links.sec=tati.idsentence where ((tat.lang='jbo' and tati.lang='eng') or (tat.lang='jbo' and tati.lang='eng')) group by tat.sentence,tati.sentence limit 1000000", (err, {lan, one, six}) => { //generate eng-jbo dict. this query is fine, just make sure that jbo is in the first column, otherwise fliflop
      if (lan === 'jbo') {
        wstream.write(`<tr><td align='right'>${one}</td><td>${six || ""}</tr>\n`);
      } else {
        wstream.write(`<tr><td align='right'>${six || ""}</td><td>${one}</tr>\n`);
      }
    });
    db.run("COMMIT");
  });
...
function tempo() {
  ...
  db.serialize(() => {
    db.run("BEGIN TRANSACTION");
    db.each("SELECT tat.idsentence as one,tat.mod as two, tat.username as three FROM tatoeba as tat where (tat.lang='jbo' and tat.username='gleki') group by one order by one asc limit 1000000", (err, {two, one}) => {
      //tat.lang='fra' and a.ids is not null
      //if (; ===null && row.four ==='OK'){
      dat = new Date(two.replace(/ /, "T"));
      if (dat < init) {
        wstream.write(`${one}\n`);
      }
      //}
...
function gettagsstats() {
  ...
  db.serialize(() => {
    db.run("BEGIN TRANSACTION");
    db.each("SELECT tag.tag as tagg, count(tag.tag) as cou, tat.lang as lan FROM tags as tag join tatoeba as tat on (tag.ids = tat.idsentence) group by tagg, lan order by lan, cou desc limit 1000000", (err, {tagg, lan, cou}) => {
      if (tagg !== null) {
        wstream.write(`${lan}\t${cou}\t${tagg}\n`);
      }
    });
    db.run("COMMIT");
  });
...
function add_finish(db, LineByLineReader, stmts) {
  db.serialize(() => {
    ...
    db.each("SELECT name FROM sqlite_master WHERE type = 'table'", (err, {name}) => {
      console.log(name);
    });
    db.close();
    /* now select */
...
function download_sentences() {
  ...
  requestd({uri, method: "GET"}).on("error", err => {}).pipe(fs.createWriteStream(ttemp)).on("finish", () => {
    ...
    const tarball = require('../../util/tarball-extract/tarball.js');
    tarball.extractTarball(ttemp, tbakfu, err => {
      if (err)
        console.log(err);
      }
    );
...
function download_links() {
  ...
  requestd({uri, method: "GET"}).on("error", err => {}).pipe(fs.createWriteStream(tlinktemp)).on("finish", () => {
    ...
    tarball.extractTarball(tlinktemp, tlinkbakfu, err => {
      if (err)
        console.log(err);
      }
    );
...
function download_tags() {
  ...
  requestd({uri, method: "GET"}).on("error", err => {}).pipe(fs.createWriteStream(ttagstemp)).on("finish", () => {
    ...
    const tarball = require('tarball-extract');
    tarball.extractTarball(ttagstemp, ttagsbakfu, err => {
      if (err)
        console.log(err);
      }
    );
