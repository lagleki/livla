...
while (cmima--) {
var def = sorcu[bau][cmima];//todo: select distinct entries from different sutysiskus. priority is having rafsi
  var i=(def.r||[]).length;
  while (i--) {
    rafsi[(def.r[i]||'')] = def;
  }
...
window.storecache=sorcu[bau].map(function(a){
...
function sisku(query, callback) {
  ...
  function be(kil,lu,e){
    ...
    if (luj){
      ...
      if(luj[0]==="@"){
        luj.shift();
        kim=luj.slice();
        for (var w=0;w<sorcu[bau].length;w++){
          var def = sorcu[bau][w];
  ...
  function shortget(a,ki,shi){
    ...
    var isdef = sorcu[bau].filter(function (o){
  ...
  function cnanosisku(lo_matra_cu_cupra,multi){
    ...
    for (var w=0;w<sorcu[bau].length;w++){
      ...
      if(m.indexOf(query.toLowerCase())>=0||m.indexOf(query.toLowerCase().replace(/h/g,"'"))>=0){
        lo_matra_cu_cupra.push(sorcu[bau][w]);
      }
    }
  ...
  {
    preciseMatches = julne(sortthem(sorcu[bau].filter(function(val){return (val.w.match(query.toLowerCase())||[]).length > 0;}).splice(0,100)));
  }
