...
function search(query, callback) {
	...
	var lastMatches = [];
	var queryP,queryF;
	Array.prototype.sortthem = function (){
		for (var i=0;i<this.length;i++) {
			var doc = restore(this[i]);//todo: optimize for phrases
			if (!doc) continue;
			...
			//log(p(doc.w) + " /// " +p(queryR) + reversal + "+++"+sli + "///"+left + ">>>"+right);
			...
			if (doc.w.willify() === query){
				...
				satisfactoryMatches.push(doc);
			}
			else if(queryR 
					&& (docw[0].match(queryR[0])||[]).length>0
					&& (docw[1].match(queryR[1])||[]).length>0
					){
				freeMatches.push(doc);
			}			
			else {
	...
	if(queryR.length===2){//for one syllable query make precise search, otherwise do below
		preciseMatches = documentStore
		.filter(
			function(val){
				var queryRn=val.w.willify().replace(/([aeiouǎąęǫ])/g,'$1-').split("-").slice(-3);
				return queryRn.length===2 ? /.*([aeiouǎąęǫ])/.exec(queryRn[0])[1] === queryR[0] : false;
			})
		...
		queryP=queryP.replace(/[rlmn]/g,'[rlmn]');
		//queryP=queryP.replace(/[mnl]/g,'[mnl]');
		queryP=queryP.replace(/[įw]/g,'[įw]');
		preciseMatches = documentStore
		.filter(
			function(val){
				return (val.w.willify()
				.match(queryP.toLowerCase()+"$")||[]).length > 0;
			})
		.filter(function(n){return n !== undefined })
		.sortthem();//now we have all matches, even poor ones
	}
	//todo: i$ doesn't rhyme with [aou]$ and vice versa, e$ - [ou]$
