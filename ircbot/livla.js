...
lin=lin.toLowerCase().trim();
	...
	["pa","ne"],["re","to"],["ci","te"],["vo","fo"],["mu","fe"],["xa","so"],["ze","se"],["bi","vo"],["so","ve"],["no","ni"],
	...
	try{
		...
		for (var j=0;j<lin.length;j++){
			if (lin[j].match(myregexp)!==null){
				lin[j]=lin[j].replace(myregexp,items[i][1])+"A";
			}
		}
		}
		lin=lin.join(" ").replace(/A/gm,"");
	}else
	{
		for (i=0;i<items.length;i++)
		{
		myregexp = new RegExp("\\b"+items[i][1]+"\\b", "gim");
