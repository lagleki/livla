...
var xugismu = function(str){
	return cmaxes.parse(str).toString().match(/^gismu,[^,]+$/) !==null ? true : false;
};
var xulujvo = function(str){
	return cmaxes.parse(str).toString().match(/^lujvo,[^,]+$/) !==null ? true : false;
};
var xufuhivla = function(str){
	return cmaxes.parse(str).toString().match(/^fuhivla,[^,]+$/) !==null ? true : false;
};
var xucmavo = function(str){
	return cmaxes.parse(str).toString().match(/^cmavo,[^,]+$/) !==null ? true : false;
};

var xucmavogunma = function(str){
	return cmaxes.parse(str).toString().match(/^(cmavo,[^,]+){2,}$/) !==null ? true : false;
};
...
var rev = xmlDoc.find("/dictionary/direction[1]/valsi");
	for (var i=0;i<rev.length;i++) {
		...
		pars+="{\"w\":\""+hi+"\"";
		try{
			if(rev[i].attr("type").value()!=='gismu' && xucmavo(hi)===false && xulujvo(hi)===false && xufuhivla(hi)===false){
