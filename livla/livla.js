...
const langs = [ 'jbo', 'en', 'ru', 'es', 'fr', 'pl', 'ja', 'de', 'eo', 'zh', 'en-simple', 'fr-facile', 'hu', 'sv' ];
const robangu = 'fr-facile|en|ru|pl|de|ja|jbo|guaspi|loglan|eo|fr|2002|es|zh|sv|en-simple|krasi|dukti|laadan|toki';
...
const katna = (lin, lng, xmlDoc) => {
	...
	if (!l) return '';
	const a = l.join(' ');
	const b = lng !== 'jbo' ? lojban.gloss(a, lng, xmlDoc, false).join(tersepli) : a;
	return `${lin} ≈ ${b}`;
};
...
async function updatexmldumps() {
	...
	for (const lang in langs) {
		const uri = `http://jbovlaste.lojban.org/export/xml-export.html?lang=${lang}`;
		jar.setCookie(cookie, uri);
		const t = path.join(__dirname, '../dumps', `${lang}.xml`);
		try {
	...
	for (const dump of [ '2002', 'en-pt-BR', 'zamenhofo', 'ile', 'ina', 'toki', 'ktv-eng', 'ldp' ].concat(langs)) {
		try {
...
const wordnet = (source, socket, clientmensi, sendTo, te_gerna) => {
	...
	wn.lookup(te_gerna, defs => {
		if (!defs || defs.length === 0) benji(source, socket, clientmensi, sendTo, '[not found]');
		defs.forEach(w => {
...
function cpedu_fi_la_arxivo(str, max) {
	...
	const db = new Database(path.join(__dirname, '../lojbo.arxivo', 'messages.sql'));
...
const processormensi = (clientmensi, from, towhom, text, message, source, socket) => {
	if (!text) return;
	let sendTo = towhom.indexOf('#') ? from : towhom;
	if (text.match(/^<(.*?)>: /, '') !== null) {
		//dealing with Slack
		from = text.match(/^<(.*?)>: /, '')[1];
		text = text.replace(/^<.*?>: /, '');
		...
		switch (true) {
			...
			default:
				...
				benji(
					...
					clientmensi,
					sendTo,
					`${from}: via Pepper's magic of Chaosah this will be sent to ${text.substr(0, text.indexOf('\t'))} after they return to the chat`
				);
		...
		switch (true) {
			...
			default:
				...
				benji(
					...
					clientmensi,
					sendTo,
					`${from}: mi ba benji di'u ba lo nu la'o gy.${text.substr(0, text.indexOf('\t'))}.gy. di'a cusku da`
				);
				fs.writeFileSync(notcijudri, notci.join('\n'));
			//loadNotci();
		}
			...
			switch (true) {
				...
				case txt.search("(.i |i |)ma rafsi zo [a-z']+") === 0:
					...
					benji(
						...
						clientmensi,
						sendTo,
						rafsi_giho_nai_se_rafsi_gui(rg.exec(text)[1].replace(/[^a-z'\.]/g, ''))
					);
					break;
		...
		default:
			switch (true) {
				...
				case txt.indexOf(`${replier}: ko ningau`) === 0 || txt.indexOf(`${replier}: ko cnino`) === 0:
					(async () => {
						benji(source, socket, clientmensi, sendTo, "sei ca ca'o jai gau cnino be fai le pe mi sorcu");
						...
						if (erroredLangs.length > 0)
							benji(
								...
								clientmensi,
								sendTo,
								`i na kakne le ka jai gau cnino fai la'e zoi zoi ${erroredLangs.join(' ')} zoi`
							);
					})();
					break;
				case txt.search(`(${robangu}):`) === 0:
				...
				case txt.search("ra'oi [a-z']+ rafsi ma") === 0:
					...
					benji(
						...
						clientmensi,
						sendTo,
						rafsi_giho_nai_se_rafsi_gui(reg.exec(text)[1].replace(/[^a-z'\.]/g, ''))
					);
					break;
				case txt.search("(.i |i |)ma rafsi zo [a-z']+") === 0:
					const rg = /.*ma rafsi zo ([a-z']+).*/;
...
clientmensi.on('message', (from, to, text) => {
