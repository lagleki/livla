...
const enxml = path.join(__dirname, "../dumps", "en" + ".xml");
let xmlDocEn = libxmljs.parseXml(fs.readFileSync(enxml, {encoding: 'utf8'}).replace(/(&lt;|<)script.*?(&gt;|>).*?(&lt;|<)/g, "&lt;").replace(/(&lt;|<)\/script(&gt;|>)/g, ""));

//the list of JVS gismu
const all_jvs_off_gismu = xmlDocEn.find(`/dictionary/direction[1]/valsi[(@type="gismu")]`).map(i => i.attr("word").value());
const all_jvs_exp_gismu = xmlDocEn.find(`/dictionary/direction[1]/valsi[(@type="experimental gismu")]`).map(i => i.attr("word").value());
...
function WhichIsInConflictAll(valsi){
  const off = WhichIsInConflict(valsi,all_jvs_off_gismu)
  const exp = WhichIsInConflict(valsi,all_jvs_exp_gismu)
  return [off,exp]
}
