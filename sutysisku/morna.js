...
langs.forEach(lang => {
  ...
  const config = JSON.parse(
    fs.readFileSync(path.join(__dirname, "../build/sutysisku/", lang, "config.json"), {
      encoding: "utf8"
    })
  );
  ...
  const output = template
    ...
    .replace(/<!--[\s\S]*?-->/gm, "")
    .replace(/\n\s*\n/g, "\n");
  fs.writeFileSync(path.join(__dirname, "../build/sutysisku/", lang, "index.html"), output);
  ...
  if (lang !== "cipra") {
    ...
    const b = sisku
      ...
      .replaceMergefield(config);
    fs.writeFileSync(path.join(__dirname, "../build/sutysisku/", lang, "sisku.xml"), b);
    //now update manifest
